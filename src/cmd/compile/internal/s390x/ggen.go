...
func (pstate *PackageState) zerorange(pp *gc.Progs, p *obj.Prog, off, cnt int64, _ *uint32) *obj.Prog {
	...
	off += pstate.gc.Ctxt.FixedFrameSize()
	...
	if off < 0 || off > 4096-clearLoopCutoff || cnt > clearLoopCutoff {
		p = pp.Appendpp(pstate.gc, p, s390x.AADD, obj.TYPE_CONST, 0, off, obj.TYPE_REG, s390x.REGRT1, 0)
	...
	if cnt > clearLoopCutoff {
		...
		p = pp.Appendpp(pstate.gc, p, s390x.AADD, obj.TYPE_CONST, 0, off+n, obj.TYPE_REG, end, 0)
		p.Reg = reg
		p = pp.Appendpp(pstate.gc, p, s390x.ACLEAR, obj.TYPE_CONST, 0, 256, obj.TYPE_MEM, reg, off)
		pl := p
		p = pp.Appendpp(pstate.gc, p, s390x.AADD, obj.TYPE_CONST, 0, 256, obj.TYPE_REG, reg, 0)
		p = pp.Appendpp(pstate.gc, p, s390x.ACMP, obj.TYPE_REG, reg, 0, obj.TYPE_REG, end, 0)
		p = pp.Appendpp(pstate.gc, p, s390x.ABNE, obj.TYPE_NONE, 0, 0, obj.TYPE_BRANCH, 0, 0)
		pstate.gc.Patch(p, pl)
	...
	for cnt > 0 {
		...
		case 8, 4, 2, 1:
			...
			p = pp.Appendpp(pstate.gc, p, ins, obj.TYPE_CONST, 0, 0, obj.TYPE_MEM, reg, off)

		// Handle clears that would require multiple move instructions with CLEAR (assembled as XC).
		default:
			p = pp.Appendpp(pstate.gc, p, s390x.ACLEAR, obj.TYPE_CONST, 0, n, obj.TYPE_MEM, reg, off)
		}
...
func (pstate *PackageState) zeroAuto(pp *gc.Progs, n *gc.Node) {
	...
	for i := int64(0); i < size; i += int64(pstate.gc.Widthptr) {
		p := pp.Prog(pstate.gc, s390x.AMOVD)
