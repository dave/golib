...
func (psess *PackageSession) zerorange(pp *gc.Progs, p *obj.Prog, off, cnt int64, _ *uint32) *obj.Prog {
	...
	if cnt < int64(4*psess.gc.Widthptr) {
		for i := int64(0); i < cnt; i += int64(psess.gc.Widthptr) {
			p = pp.Appendpp(psess.gc, p, mips.AMOVV, obj.TYPE_REG, mips.REGZERO, 0, obj.TYPE_MEM, mips.REGSP, 8+off+i)
		}
	} else if cnt <= int64(128*psess.gc.Widthptr) {
		p = pp.Appendpp(psess.gc, p, mips.AADDV, obj.TYPE_CONST, 0, 8+off-8, obj.TYPE_REG, mips.REGRT1, 0)
		p.Reg = mips.REGSP
		p = pp.Appendpp(psess.gc, p, obj.ADUFFZERO, obj.TYPE_NONE, 0, 0, obj.TYPE_MEM, 0, 0)
		p.To.Name = obj.NAME_EXTERN
		p.To.Sym = psess.gc.Duffzero
		p.To.Offset = 8 * (128 - cnt/int64(psess.gc.Widthptr))
	} else {

		p = pp.Appendpp(psess.gc, p, mips.AADDV, obj.TYPE_CONST, 0, 8+off-8, obj.TYPE_REG, mips.REGRT1, 0)
		p.Reg = mips.REGSP
		p = pp.Appendpp(psess.gc, p, mips.AADDV, obj.TYPE_CONST, 0, cnt, obj.TYPE_REG, mips.REGRT2, 0)
		p.Reg = mips.REGRT1
		p = pp.Appendpp(psess.gc, p, mips.AMOVV, obj.TYPE_REG, mips.REGZERO, 0, obj.TYPE_MEM, mips.REGRT1, int64(psess.gc.Widthptr))
		p1 := p
		p = pp.Appendpp(psess.gc, p, mips.AADDV, obj.TYPE_CONST, 0, int64(psess.gc.Widthptr), obj.TYPE_REG, mips.REGRT1, 0)
