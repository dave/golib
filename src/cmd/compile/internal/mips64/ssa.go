...
func (psess *PackageSession) ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpMIPS64MOVBreg,
		...
		if a.Op == ssa.OpLoadReg {
			t := a.Type
			switch {
			case v.Op == ssa.OpMIPS64MOVBreg && t.Size(psess.types) == 1 && t.IsSigned(),
				v.Op == ssa.OpMIPS64MOVBUreg && t.Size(psess.types) == 1 && !t.IsSigned(),
				v.Op == ssa.OpMIPS64MOVHreg && t.Size(psess.types) == 2 && t.IsSigned(),
				v.Op == ssa.OpMIPS64MOVHUreg && t.Size(psess.types) == 2 && !t.IsSigned(),
				v.Op == ssa.OpMIPS64MOVWreg && t.Size(psess.types) == 4 && t.IsSigned(),
				v.Op == ssa.OpMIPS64MOVWUreg && t.Size(psess.types) == 4 && !t.IsSigned():
	...
	case ssa.OpMIPS64LoweredZero:
		...
		p2.To.Offset = sz
		p3 := s.Prog(psess.gc, mips.AADDVU)
	...
	case ssa.OpMIPS64LoweredMove:
		...
		p4 := s.Prog(psess.gc, mips.AADDVU)
		p4.From.Type = obj.TYPE_CONST
		p4.From.Offset = sz
		p4.To.Type = obj.TYPE_REG
		p4.To.Reg = mips.REG_R1
		p5 := s.Prog(psess.gc, mips.AADDVU)
	...
	case ssa.OpMIPS64LoweredAtomicAdd32, ssa.OpMIPS64LoweredAtomicAdd64:
		...
		s.Prog(psess.gc, mips.ASYNC)
		p := s.Prog(psess.gc, ll)
		p.From.Type = obj.TYPE_MEM
		p.From.Reg = v.Args[0].Reg(psess.ssa)
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg0(psess.ssa)
		p1 := s.Prog(psess.gc, mips.AADDVU)
	...
	case ssa.OpMIPS64FPFlagTrue,
		...
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg(psess.ssa)
		p2 := s.Prog(psess.gc, branch)
		p2.To.Type = obj.TYPE_BRANCH
		p3 := s.Prog(psess.gc, mips.AMOVV)
		p3.From.Type = obj.TYPE_CONST
		p3.From.Offset = 1
		p3.To.Type = obj.TYPE_REG
		p3.To.Reg = v.Reg(psess.ssa)
		p4 := s.Prog(psess.gc, obj.ANOP)
		psess.gc.
			Patch(p2, p4)
