...
func (pstate *PackageState) typecheckswitch(n *Node) {
	...
	if n.Left != nil && n.Left.Op == OTYPESW {
		// type switch
		top = Etype
		n.Left.Right = pstate.typecheck(n.Left.Right, Erv)
		t = n.Left.Right.Type
		if t != nil && !t.IsInterface() {
			pstate.yyerrorl(n.Pos, "cannot type switch on non-interface value %L", n.Left.Right)
		}
		if v := n.Left.Left; v != nil && !v.isBlank() && n.List.Len() == 0 {
			// We don't actually declare the type switch's guarded
			// declaration itself. So if there are no cases, we
			// won't notice that it went unused.
			pstate.yyerrorl(v.Pos, "%v declared and not used", v.Sym)
		}
	} else {
		// expression switch
		top = Erv
		...
		if t != nil {
			switch {
			case !pstate.okforeq[t.Etype]:
				pstate.yyerrorl(n.Pos, "cannot switch on %L", n.Left)
			case t.IsSlice():
				nilonly = "slice"
			case t.IsArray() && !pstate.IsComparable(t):
				pstate.yyerrorl(n.Pos, "cannot switch on %L", n.Left)
			case t.IsStruct():
				if f := pstate.IncomparableField(t); f != nil {
					pstate.yyerrorl(n.Pos, "cannot switch on %L (struct containing %v cannot be compared)", n.Left, f.Type)
				}
			case t.Etype == TFUNC:
				nilonly = "func"
			case t.IsMap():
	...
	for _, ncase := range n.List.Slice() {
		if ncase.List.Len() == 0 {
			...
			if def != nil {
				...
				pstate.yyerrorl(ncase.Pos, "multiple defaults in switch (first at %v)", def.Line(pstate))
			} else {
				def = ncase
			}
		} else {
			...
			for i1, n1 := range ls {
				pstate.setlineno(n1)
				ls[i1] = pstate.typecheck(ls[i1], Erv|Etype)
				n1 = ls[i1]
				...
				// expression switch
				case Erv:
					ls[i1] = pstate.defaultlit(ls[i1], t)
					n1 = ls[i1]
					switch {
					case n1.Op == OTYPE:
						pstate.yyerrorl(ncase.Pos, "type %v is not an expression", n1.Type)
					case n1.Type != nil && pstate.assignop(n1.Type, t, nil) == 0 && pstate.assignop(t, n1.Type, nil) == 0:
						if n.Left != nil {
							pstate.yyerrorl(ncase.Pos, "invalid case %v in switch on %v (mismatched types %v and %v)", n1, n.Left, n1.Type, t)
						} else {
							pstate.yyerrorl(ncase.Pos, "invalid case %v in switch (mismatched types %v and bool)", n1, n1.Type)
						}
					case nilonly != "" && !n1.isNil(pstate):
						pstate.yyerrorl(ncase.Pos, "invalid case %v in switch (can only compare %s %v to nil)", n1, nilonly, n.Left)
					case t.IsInterface() && !n1.Type.IsInterface() && !pstate.IsComparable(n1.Type):
						pstate.yyerrorl(ncase.Pos, "invalid case %L in switch (incomparable type)", n1)
					}
				...
				case Etype:
					...
					case n1.Op == OLITERAL && n1.Type.IsKind(TNIL):
						// case nil:
						if niltype != nil {
							pstate.yyerrorl(ncase.Pos, "multiple nil cases in type switch (first at %v)", niltype.Line(pstate))
						} else {
							niltype = ncase
						}
					case n1.Op != OTYPE && n1.Type != nil: // should this be ||?
						pstate.yyerrorl(ncase.Pos, "%L is not a type", n1)
						// reset to original type
						n1 = n.Left.Right
						ls[i1] = n1
					case !n1.Type.IsInterface() && t.IsInterface() && !pstate.implements(n1.Type, t, &missing, &have, &ptr):
						if have != nil && !missing.Broke() && !have.Broke() {
							pstate.yyerrorl(ncase.Pos, "impossible type switch case: %L cannot have dynamic type %v"+
		...
		if n.Type == nil || n.Type.IsUntyped(pstate.types) {
		...
		if top == Etype {
			...
			if ncase.Rlist.Len() != 0 {
				...
				nvar = pstate.typecheck(nvar, Erv|Easgn)
	...
	// expression switch
	case Erv:
		pstate.checkDupExprCases(n.Left, n.List.Slice())
	}
}

// walkswitch walks a switch statement.
func (pstate *PackageState) walkswitch(sw *Node) {
	// convert switch {...} to switch true {...}
	if sw.Left == nil {
		sw.Left = pstate.nodbool(true)
		sw.Left = pstate.typecheck(sw.Left, Erv)
		sw.Left = pstate.defaultlit(sw.Left, nil)
	}
...
func (s *exprSwitch) walk(pstate *PackageState, sw *Node) {
	// Guard against double walk, see #25776.
	if sw.List.Len() == 0 && sw.Nbody.Len() > 0 {
		pstate.Fatalf("second walk of switch")
	}

	pstate.casebody(sw, nil)

	cond := sw.Left
	sw.Left = nil

	s.kind = switchKindExpr
	if pstate.Isconst(cond, CTBOOL) {
	...
	if cond.Op == OARRAYBYTESTR {
		...
		for _, cas := range sw.List.Slice() {
			if cas.Op != OCASE {
				pstate.Fatalf("switch string(byteslice) bad op: %v", cas.Op)
			}
			if cas.Left != nil && !pstate.Isconst(cas.Left, CTSTR) {
				ok = false
				break
			}
		}
	...
	cond = pstate.walkexpr(cond, &sw.Ninit)
	t := sw.Type
	if t == nil {
		return
	}
	...
	if s.kind == switchKindTrue || s.kind == switchKindFalse {
		s.exprname = pstate.nodbool(s.kind == switchKindTrue)
	} else if pstate.consttype(cond) > 0 {
		// leave constants to enable dead code elimination (issue 9608)
		s.exprname = cond
	} else {
		s.exprname = pstate.temp(cond.Type)
		cas = []*Node{pstate.nod(OAS, s.exprname, cond)}
		pstate.typecheckslice(cas, Etop)
	}

	// Enumerate the cases and prepare the default case.
	clauses := s.genCaseClauses(pstate, sw.List.Slice())
	sw.List.Set(nil)
	cc := clauses.list
	...
	for len(cc) > 0 {
		run := 1
		if pstate.okforcmp[t.Etype] && cc[0].isconst {
	...
	if pstate.nerrors == 0 {
		...
		pstate.walkstmtlist(sw.Nbody.Slice())
	}
}

// walkCases generates an AST implementing the cases in cc.
func (s *exprSwitch) walkCases(pstate *PackageState, cc []caseClause) *Node {
	if len(cc) < binarySearchMin {
		...
		for _, c := range cc {
			...
			if rng := n.List.Slice(); rng != nil {
				...
				low := pstate.nod(OGE, s.exprname, rng[0])
				high := pstate.nod(OLE, s.exprname, rng[1])
				a.Left = pstate.nod(OANDAND, low, high)
			} else if (s.kind != switchKindTrue && s.kind != switchKindFalse) || pstate.assignop(n.Left.Type, s.exprname.Type, nil) == OCONVIFACE || pstate.assignop(s.exprname.Type, n.Left.Type, nil) == OCONVIFACE {
				a.Left = pstate.nod(OEQ, s.exprname, n.Left) // if name == val
			} else if s.kind == switchKindTrue {
				a.Left = n.Left // if val
			} else {
				// s.kind == switchKindFalse
				a.Left = pstate.nod(ONOT, n.Left, nil) // if !val
			}
			a.Left = pstate.typecheck(a.Left, Erv)
			a.Left = pstate.defaultlit(a.Left, nil)
			a.Nbody.Set1(n.Right) // goto l

			cas = append(cas, a)
			pstate.lineno = lno
		}
		return pstate.liststmt(cas)
	}
	...
	le := pstate.nod(OLE, s.exprname, mid)
	if pstate.Isconst(mid, CTSTR) {
		// Search by length and then by value; see caseClauseByConstVal.
		lenlt := pstate.nod(OLT, pstate.nod(OLEN, s.exprname, nil), pstate.nod(OLEN, mid, nil))
		leneq := pstate.nod(OEQ, pstate.nod(OLEN, s.exprname, nil), pstate.nod(OLEN, mid, nil))
		a.Left = pstate.nod(OOROR, lenlt, pstate.nod(OANDAND, leneq, le))
	} else {
		a.Left = le
	}
	a.Left = pstate.typecheck(a.Left, Erv)
	a.Left = pstate.defaultlit(a.Left, nil)
	a.Nbody.Set1(s.walkCases(pstate, cc[:half]))
	a.Rlist.Set1(s.walkCases(pstate, cc[half:]))
	return a
}
...
func (pstate *PackageState) casebody(sw *Node, typeswvar *Node) {
	if sw.List.Len() == 0 {
		return
	}

	lno := pstate.setlineno(sw)

	var cas []*Node  // cases
	var stat []*Node // statements
	var def *Node    // defaults
	br := pstate.nod(OBREAK, nil, nil)

	for _, n := range sw.List.Slice() {
		pstate.setlineno(n)
		if n.Op != OXCASE {
			pstate.Fatalf("casebody %v", n.Op)
		}
		n.Op = OCASE
		needvar := n.List.Len() != 1 || n.List.First().Op == OLITERAL

		jmp := pstate.nod(OGOTO, pstate.autolabel(".s"), nil)
		switch n.List.Len() {
		case 0:
			// default
			if def != nil {
				pstate.yyerrorl(n.Pos, "more than one default case")
			}
		...
		default:
			...
			if typeswvar != nil || sw.Left.Type.IsInterface() || !n.List.First().Type.IsInteger() || n.List.Len() < integerRangeMin {
				...
				for _, n1 := range n.List.Slice() {
					cas = append(cas, pstate.nod(OCASE, n1, jmp))
				}
				break
			}
			...
			for j < len(s) {
				...
				var run int
				for run = j; run < len(s) && pstate.Isconst(s[run], CTINT); run++ {
				}
				...
				for ; j < len(s) && (j < run || !pstate.Isconst(s[j], CTINT)); j++ {
					cas = append(cas, pstate.nod(OCASE, s[j], jmp))
				}
			}
		}

		stat = append(stat, pstate.nod(OLABEL, jmp.Left, nil))
		if typeswvar != nil && needvar && n.Rlist.Len() != 0 {
			l := []*Node{
				pstate.nod(ODCL, n.Rlist.First(), nil),
				pstate.nod(OAS, n.Rlist.First(), typeswvar),
			}
...
func (s *exprSwitch) genCaseClauses(pstate *PackageState, clauses []*Node) caseClauses {
	var cc caseClauses
	for _, n := range clauses {
		if n.Left == nil && n.List.Len() == 0 {
			// default case
			if cc.defjmp != nil {
				pstate.Fatalf("duplicate default case not detected during typechecking")
			}
			cc.defjmp = n.Right
			continue
		}
		...
		switch pstate.consttype(n.Left) {
	...
	if cc.defjmp == nil {
		cc.defjmp = pstate.nod(OBREAK, nil, nil)
	}
	return cc
}

// genCaseClauses generates the caseClauses value for clauses.
func (s *typeSwitch) genCaseClauses(pstate *PackageState, clauses []*Node) caseClauses {
	var cc caseClauses
	for _, n := range clauses {
		switch {
		case n.Left == nil:
		...
		case n.Left.Op == OLITERAL:
			...
			if cc.niljmp != nil {
				pstate.Fatalf("duplicate nil case not detected during typechecking")
			}
			cc.niljmp = n.Right
			continue
		}
		...
		c := caseClause{
			...
			hash:    pstate.typehash(n.Left.Type),
		}
		cc.list = append(cc.list, c)
	}
	...
	s.checkDupCases(pstate, cc.list)
	return cc
}

func (s *typeSwitch) checkDupCases(pstate *PackageState, cc []caseClause) {
	if len(cc) < 2 {
		return
	}
...
Outer:
	for _, c := range cc {
		...
		for _, n := range prev {
			if pstate.eqtype(n.Left.Type, c.node.Left.Type) {
				pstate.yyerrorl(c.node.Pos, "duplicate case %v in type switch\n\tprevious case at %v", c.node.Left.Type, n.Line(pstate))
...
func (pstate *PackageState) checkDupExprCases(exprname *Node, clauses []*Node) {
	...
	if !exprname.Type.IsInterface() {
		...
		for _, ncase := range clauses {
			for _, n := range ncase.List.Slice() {
				...
				if ct := pstate.consttype(n); ct == 0 || ct == CTBOOL {
					continue
				}

				val := n.Val().Interface(pstate)
				prev, dup := seen[val]
				if !dup {
					seen[val] = n
					continue
				}
				pstate.yyerrorl(ncase.Pos, "duplicate case %s in switch\n\tprevious case at %v",
					pstate.nodeAndVal(n), prev.Line(pstate))
			}
		}
		return
	}
	...
	for _, ncase := range clauses {
		for _, n := range ncase.List.Slice() {
			...
			switch tv.typ {
			case pstate.types.Bytetype:
				tv.typ = pstate.types.Types[TUINT8]
			case pstate.types.Runetype:
				tv.typ = pstate.types.Types[TINT32]
			}
			prev, dup := seen[tv]
			if !dup {
				seen[tv] = n
...
func (pstate *PackageState) nodeAndVal(n *Node) string {
	show := n.String()
	val := n.Val().Interface(pstate)
...
func (s *typeSwitch) walk(pstate *PackageState, sw *Node) {
	cond := sw.Left
	sw.Left = nil
	...
	if cond.Right == nil {
		pstate.yyerrorl(sw.Pos, "type switch must have an assignment")
		return
	}

	cond.Right = pstate.walkexpr(cond.Right, &sw.Ninit)
	if !cond.Right.Type.IsInterface() {
		pstate.yyerrorl(sw.Pos, "type switch must be on an interface")
		return
	}
	...
	s.facename = pstate.temp(cond.Right.Type)

	a := pstate.nod(OAS, s.facename, cond.Right)
	a = pstate.typecheck(a, Etop)
	cas = append(cas, a)

	s.okname = pstate.temp(pstate.types.Types[TBOOL])
	s.okname = pstate.typecheck(s.okname, Erv)

	s.hashname = pstate.temp(pstate.types.Types[TUINT32])
	s.hashname = pstate.typecheck(s.hashname, Erv)

	// set up labels and jumps
	pstate.casebody(sw, s.facename)
	...
	itab := pstate.nod(OITAB, s.facename, nil)

	// Check for nil first.
	i := pstate.nod(OIF, nil, nil)
	i.Left = pstate.nod(OEQ, itab, pstate.nodnil())
	if clauses.niljmp != nil {
		// Do explicit nil case right here.
		i.Nbody.Set1(clauses.niljmp)
	} else {
		// Jump to default case.
		lbl := pstate.autolabel(".s")
		i.Nbody.Set1(pstate.nod(OGOTO, lbl, nil))
		// Wrap default case with label.
		blk := pstate.nod(OBLOCK, nil, nil)
		blk.List.Set2(pstate.nod(OLABEL, lbl, nil), def)
		def = blk
	}
	i.Left = pstate.typecheck(i.Left, Erv)
	i.Left = pstate.defaultlit(i.Left, nil)
	cas = append(cas, i)

	// Load hash from type or itab.
	h := pstate.nodSym(ODOTPTR, itab, nil)
	h.Type = pstate.types.Types[TUINT32]
	h.SetTypecheck(1)
	if cond.Right.Type.IsEmptyInterface(pstate.types) {
		h.Xoffset = int64(2 * pstate.Widthptr) // offset of hash in runtime._type
	} else {
		h.Xoffset = int64(2 * pstate.Widthptr) // offset of hash in runtime.itab
	}
	h.SetBounded(true) // guaranteed not to fault
	a = pstate.nod(OAS, s.hashname, h)
	a = pstate.typecheck(a, Etop)
	cas = append(cas, a)
	...
	for _, c := range cc {
		c.node.Right = s.typeone(pstate, c.node)
	}
	...
	for len(cc) > 0 {
		...
		for i, j := 0, 0; i < run; i = j {
			...
			cc[i].node.Right = pstate.liststmt(hash)
			batch = append(batch, cc[i])
		}

		// binary search among cases to narrow by hash
		cas = append(cas, s.walkCases(pstate, batch))
		cc = cc[run:]
	}
...
func (s *typeSwitch) typeone(pstate *PackageState, t *Node) *Node {
	var name *Node
	var init Nodes
	if t.Rlist.Len() == 0 {
		name = pstate.nblank
		...
		name = t.Rlist.First()
		init.Append(pstate.nod(ODCL, name, nil))
		a := pstate.nod(OAS, name, nil)
		a = pstate.typecheck(a, Etop)
		init.Append(a)
	}
	...
	b := pstate.nod(ODOTTYPE, s.facename, nil)
	...
	c := pstate.nod(OIF, nil, nil)
	c.Left = s.okname
	c.Nbody.Set1(t.Right) // if ok { goto l }

	init.Append(c)
	return init.asblock(pstate)
}

// walkCases generates an AST implementing the cases in cc.
func (s *typeSwitch) walkCases(pstate *PackageState, cc []caseClause) *Node {
	if len(cc) < binarySearchMin {
		var cas []*Node
		for _, c := range cc {
			n := c.node
			if !c.isconst {
				pstate.Fatalf("typeSwitch walkCases")
			}
			a := pstate.nod(OIF, nil, nil)
			a.Left = pstate.nod(OEQ, s.hashname, pstate.nodintconst(int64(c.hash)))
			a.Left = pstate.typecheck(a.Left, Erv)
			a.Left = pstate.defaultlit(a.Left, nil)
	...
	half := len(cc) / 2
	a := pstate.nod(OIF, nil, nil)
	a.Left = pstate.nod(OLE, s.hashname, pstate.nodintconst(int64(cc[half-1].hash)))
	a.Left = pstate.typecheck(a.Left, Erv)
	a.Left = pstate.defaultlit(a.Left, nil)
	a.Nbody.Set1(s.walkCases(pstate, cc[:half]))
	a.Rlist.Set1(s.walkCases(pstate, cc[half:]))
...
func (x caseClauseByConstVal) Less(pstate *PackageState, i, j int) bool {
	...
	pstate.Fatalf("caseClauseByConstVal passed bad clauses %v < %v", x[i].node.Left, x[j].node.Left)
	return false
}
