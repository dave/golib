...
func (pstate *PackageState) ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpMIPSMOVBreg,
		...
		if a.Op == ssa.OpLoadReg {
			t := a.Type
			switch {
			case v.Op == ssa.OpMIPSMOVBreg && t.Size(pstate.types) == 1 && t.IsSigned(),
				v.Op == ssa.OpMIPSMOVBUreg && t.Size(pstate.types) == 1 && !t.IsSigned(),
				v.Op == ssa.OpMIPSMOVHreg && t.Size(pstate.types) == 2 && t.IsSigned(),
				v.Op == ssa.OpMIPSMOVHUreg && t.Size(pstate.types) == 2 && !t.IsSigned():
	...
	case ssa.OpMIPSLoweredZero:
		...
		p2.To.Offset = sz
		p3 := s.Prog(pstate.gc, mips.AADDU)
	...
	case ssa.OpMIPSLoweredMove:
		...
		p4 := s.Prog(pstate.gc, mips.AADDU)
		p4.From.Type = obj.TYPE_CONST
		p4.From.Offset = sz
		p4.To.Type = obj.TYPE_REG
		p4.To.Reg = mips.REG_R1
		p5 := s.Prog(pstate.gc, mips.AADDU)
	...
	case ssa.OpMIPSLoweredAtomicAdd:
		...
		p := s.Prog(pstate.gc, mips.ALL)
		p.From.Type = obj.TYPE_MEM
		p.From.Reg = v.Args[0].Reg(pstate.ssa)
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg0(pstate.ssa)

		p1 := s.Prog(pstate.gc, mips.AADDU)
	...
	case ssa.OpMIPSFPFlagTrue,
		...
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg(pstate.ssa)
		p1 := s.Prog(pstate.gc, cmov)
		p1.From.Type = obj.TYPE_REG
		p1.From.Reg = mips.REGZERO
		p1.To.Type = obj.TYPE_REG
		p1.To.Reg = v.Reg(pstate.ssa)
