...
func (psess *PackageSession) Compile(f *Func) {
	...
	printFunc(f)
	f.HTMLWriter.WriteFunc(psess, "start", "start", f)
	if psess.BuildDump != "" && psess.BuildDump == f.Name {
		f.dumpFile(psess, "build")
	}
	...
	for _, p := range psess.passes {
		...
		if f.Log() || f.HTMLWriter != nil {
			...
			f.Logf("  pass %s end %s\n", p.name, stats)
			printFunc(f)
			f.HTMLWriter.WriteFunc(psess, phaseName, fmt.Sprintf("%s <span class=\"stats\">%s</span>", phaseName, stats), f)
		}
		if p.time || p.mem {
...
// output.
func (f *Func) dumpFile(psess *PackageSession, phaseName string) {
	psess.
		dumpFileSeq++
	fname := fmt.Sprintf("%s_%02d__%s.dump", f.Name, psess.dumpFileSeq, phaseName)
	...
	if err != nil {
		f.Warnl(psess.src.NoXPos, "Unable to create after-phase dump file %s", fname)
		return
	}
...
// name of function to dump after initial build of ssa
...
//
func (psess *PackageSession) PhaseOption(phase, flag string, val int, valString string) string {
	if phase == "help" {
		lastcr := 0
...
func (psess *PackageSession) init() {
	for _, c := range psess.passOrder {
		a, b := c.a, c.b
		i := -1
		j := -1
		for k, p := range psess.passes {
			if p.name == a {
				i = k
			}
			if p.name == b {
