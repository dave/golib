...
func (w *HTMLWriter) WriteFunc(psess *PackageSession, phase, title string, f *Func) {
	if w == nil {
		return
	}
	w.WriteColumn(psess, phase, title, "", f.HTML(psess))

}

// WriteColumn writes raw HTML in a column headed by title.
// It is intended for pre- and post-compilation log output.
func (w *HTMLWriter) WriteColumn(psess *PackageSession, phase, title, class, html string) {
	if w == nil {
		return
	}
	id := strings.Replace(phase, " ", "-", -1)

	w.Printf(psess, "<td id=\"%v-col\" class=\"collapsed\"><div>%v</div></td>", id, phase)

	if class == "" {
		w.Printf(psess, "<td id=\"%v-exp\">", id)
	} else {
		w.Printf(psess, "<td id=\"%v-exp\" class=\"%v\">", id, class)
	}
	w.WriteString(psess, "<h2>"+title+"</h2>")
	w.WriteString(psess, html)
	w.WriteString(psess, "</td>")
}

func (w *HTMLWriter) Printf(psess *PackageSession, msg string, v ...interface{}) {
	if _, err := fmt.Fprintf(w.w, msg, v...); err != nil {
		w.Fatalf(psess.src.NoXPos, "%v", err)
	}
}

func (w *HTMLWriter) WriteString(psess *PackageSession, s string) {
...
func (v *Value) LongHTML(psess *PackageSession) string {
	...
	s += fmt.Sprintf("%s %s = %s", v.HTML(), linenumber, v.Op.String(psess))

	s += " &lt;" + html.EscapeString(v.Type.String(psess.types)) + "&gt;"
	s += html.EscapeString(v.auxString(psess))
...
func (b *Block) LongHTML(psess *PackageSession) string {

	s := fmt.Sprintf("<span class=\"%s ssa-block\">%s</span>", html.EscapeString(b.String()), html.EscapeString(b.Kind.String(psess)))
	if b.Aux != nil {
...
func (f *Func) HTML(psess *PackageSession) string {
...
func (p htmlFuncPrinter) endBlock(psess *PackageSession, b *Block) {
	if len(b.Values) > 0 {
		io.WriteString(p.w, "</ul>")
		io.WriteString(p.w, "</li>")
	}
	io.WriteString(p.w, "<li class=\"ssa-end-block\">")
	fmt.Fprint(p.w, b.LongHTML(psess))
	io.WriteString(p.w, "</li>")
	io.WriteString(p.w, "</ul>")

}

func (p htmlFuncPrinter) value(psess *PackageSession, v *Value, live bool) {
	var dead string
	if !live {
		dead = "dead-value"
	}
	fmt.Fprintf(p.w, "<li class=\"ssa-long-value %s\">", dead)
	fmt.Fprint(p.w, v.LongHTML(psess))
	io.WriteString(p.w, "</li>")
}
