...
func (pstate *PackageState) ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpARM64FMADDS,
		...
		ssa.OpARM64FNMSUBD:
		rt := v.Reg(pstate.ssa)
		ra := v.Args[0].Reg(pstate.ssa)
		rm := v.Args[1].Reg(pstate.ssa)
		rn := v.Args[2].Reg(pstate.ssa)
		p := s.Prog(pstate.gc, v.Op.Asm(pstate.ssa))
		p.Reg = ra
		p.From.Type = obj.TYPE_REG
		p.From.Reg = rm
	...
	case ssa.OpARM64ADDshiftLL,
		...
		pstate.genshift(s, v.Op.Asm(pstate.ssa), v.Args[0].Reg(pstate.ssa), v.Args[1].Reg(pstate.ssa), v.Reg(pstate.ssa), arm64.SHIFT_LL, v.AuxInt)
	case ssa.OpARM64ADDshiftRL,
		...
		pstate.genshift(s, v.Op.Asm(pstate.ssa), v.Args[0].Reg(pstate.ssa), v.Args[1].Reg(pstate.ssa), v.Reg(pstate.ssa), arm64.SHIFT_LR, v.AuxInt)
	case ssa.OpARM64ADDshiftRA,
		...
		pstate.genshift(s, v.Op.Asm(pstate.ssa), v.Args[0].Reg(pstate.ssa), v.Args[1].Reg(pstate.ssa), v.Reg(pstate.ssa), arm64.SHIFT_AR, v.AuxInt)
	...
	case ssa.OpARM64CMPshiftLL:
		pstate.genshift(s, v.Op.Asm(pstate.ssa), v.Args[0].Reg(pstate.ssa), v.Args[1].Reg(pstate.ssa), 0, arm64.SHIFT_LL, v.AuxInt)
	case ssa.OpARM64CMPshiftRL:
		pstate.genshift(s, v.Op.Asm(pstate.ssa), v.Args[0].Reg(pstate.ssa), v.Args[1].Reg(pstate.ssa), 0, arm64.SHIFT_LR, v.AuxInt)
	case ssa.OpARM64CMPshiftRA:
		pstate.genshift(s, v.Op.Asm(pstate.ssa), v.Args[0].Reg(pstate.ssa), v.Args[1].Reg(pstate.ssa), 0, arm64.SHIFT_AR, v.AuxInt)
	case ssa.OpARM64MOVDaddr:
		p := s.Prog(pstate.gc, arm64.AMOVD)
	...
	case ssa.OpARM64MOVBloadidx,
		...
		p := s.Prog(pstate.gc, v.Op.Asm(pstate.ssa))
		p.From = pstate.genIndexedOperand(v)
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg(pstate.ssa)
	...
	case ssa.OpARM64MOVBstoreidx,
		...
		p := s.Prog(pstate.gc, v.Op.Asm(pstate.ssa))
		p.To = pstate.genIndexedOperand(v)
		p.From.Type = obj.TYPE_REG
	...
	case ssa.OpARM64STP:
		...
		p.From.Reg = v.Args[1].Reg(pstate.ssa)
		p.From.Offset = int64(v.Args[2].Reg(pstate.ssa))
		p.To.Type = obj.TYPE_MEM
		p.To.Reg = v.Args[0].Reg(pstate.ssa)
		pstate.gc.AddAux(&p.To, v)
	...
	case ssa.OpARM64LoweredMuluhilo:
		...
		p := s.Prog(pstate.gc, arm64.AUMULH)
		p.From.Type = obj.TYPE_REG
		p.From.Reg = r1
		p.Reg = r0
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg0(pstate.ssa)
		p1 := s.Prog(pstate.gc, arm64.AMUL)
		p1.From.Type = obj.TYPE_REG
		p1.From.Reg = r1
		p1.Reg = r0
		p1.To.Type = obj.TYPE_REG
		p1.To.Reg = v.Reg1(pstate.ssa)
	case ssa.OpARM64LoweredAtomicExchange64,
		...
		p2 := s.Prog(pstate.gc, arm64.ACBNZ)
	...
	case ssa.OpARM64LoweredAtomicAdd64,
		...
		p.To.Type = obj.TYPE_REG
		p.To.Reg = out
		p1 := s.Prog(pstate.gc, arm64.AADD)
		p1.From.Type = obj.TYPE_REG
		p1.From.Reg = r1
		p1.To.Type = obj.TYPE_REG
		p1.To.Reg = out
		p2 := s.Prog(pstate.gc, st)
		p2.From.Type = obj.TYPE_REG
		p2.From.Reg = out
		p2.To.Type = obj.TYPE_MEM
		p2.To.Reg = r0
		p2.RegTo2 = arm64.REGTMP
		p3 := s.Prog(pstate.gc, arm64.ACBNZ)
	...
	case ssa.OpARM64LoweredAtomicCas64,
		...
		p1.From.Reg = r1
		p1.Reg = arm64.REGTMP
		p2 := s.Prog(pstate.gc, arm64.ABNE)
		p2.To.Type = obj.TYPE_BRANCH
	...
	case ssa.OpARM64LoweredAtomicAnd8,
		...
		r1 := v.Args[1].Reg(pstate.ssa)
		out := v.Reg0(pstate.ssa)
		p := s.Prog(pstate.gc, arm64.ALDAXRB)
		p.From.Type = obj.TYPE_MEM
		p.From.Reg = r0
		p.To.Type = obj.TYPE_REG
		p.To.Reg = out
		...
		p1.To.Type = obj.TYPE_REG
		p1.To.Reg = out
		p2 := s.Prog(pstate.gc, arm64.ASTLXRB)
		p2.From.Type = obj.TYPE_REG
		p2.From.Reg = out
		p2.To.Type = obj.TYPE_MEM
		p2.To.Reg = r0
	...
	case ssa.OpARM64MOVBreg,
		...
		if a.Op == ssa.OpLoadReg {
			t := a.Type
			switch {
			case v.Op == ssa.OpARM64MOVBreg && t.Size(pstate.types) == 1 && t.IsSigned(),
				v.Op == ssa.OpARM64MOVBUreg && t.Size(pstate.types) == 1 && !t.IsSigned(),
				v.Op == ssa.OpARM64MOVHreg && t.Size(pstate.types) == 2 && t.IsSigned(),
				v.Op == ssa.OpARM64MOVHUreg && t.Size(pstate.types) == 2 && !t.IsSigned(),
				v.Op == ssa.OpARM64MOVWreg && t.Size(pstate.types) == 4 && t.IsSigned(),
				v.Op == ssa.OpARM64MOVWUreg && t.Size(pstate.types) == 4 && !t.IsSigned():
	...
	case ssa.OpARM64VCNT:
		p := s.Prog(pstate.gc, v.Op.Asm(pstate.ssa))
		p.From.Type = obj.TYPE_REG
		p.From.Reg = (v.Args[0].Reg(pstate.ssa)-arm64.REG_F0)&31 + arm64.REG_ARNG + ((arm64.ARNG_8B & 15) << 5)
		p.To.Type = obj.TYPE_REG
		p.To.Reg = (v.Reg(pstate.ssa)-arm64.REG_F0)&31 + arm64.REG_ARNG + ((arm64.ARNG_8B & 15) << 5)
	case ssa.OpARM64VUADDLV:
		p := s.Prog(pstate.gc, v.Op.Asm(pstate.ssa))
		p.From.Type = obj.TYPE_REG
		p.From.Reg = (v.Args[0].Reg(pstate.ssa)-arm64.REG_F0)&31 + arm64.REG_ARNG + ((arm64.ARNG_8B & 15) << 5)
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg(pstate.ssa) - arm64.REG_F0 + arm64.REG_V0
	case ssa.OpARM64CSEL, ssa.OpARM64CSEL0:
		...
		p.From.Reg = pstate.condBits[v.Aux.(ssa.Op)]
		p.Reg = v.Args[0].Reg(pstate.ssa)
	...
	case ssa.OpARM64LoweredZero:
		...
		p.To.Offset = 16
		p2 := s.Prog(pstate.gc, arm64.ACMP)
		p2.From.Type = obj.TYPE_REG
		p2.From.Reg = v.Args[1].Reg(pstate.ssa)
		p2.Reg = arm64.REG_R16
		p3 := s.Prog(pstate.gc, arm64.ABLE)
		p3.To.Type = obj.TYPE_BRANCH
		pstate.gc.Patch(p3, p)
	...
	case ssa.OpARM64LoweredMove:
		...
		p.To.Type = obj.TYPE_REG
		p.To.Reg = arm64.REGTMP
		p2 := s.Prog(pstate.gc, arm64.AMOVD)
		...
		p2.To.Offset = 8
		p3 := s.Prog(pstate.gc, arm64.ACMP)
		p3.From.Type = obj.TYPE_REG
		p3.From.Reg = v.Args[2].Reg(pstate.ssa)
		p3.Reg = arm64.REG_R16
		p4 := s.Prog(pstate.gc, arm64.ABLE)
		p4.To.Type = obj.TYPE_BRANCH
		pstate.gc.Patch(p4, p)
	...
	case ssa.OpARM64LoweredNilCheck:
		...
		p := s.Prog(pstate.gc, arm64.AMOVB)
		p.From.Type = obj.TYPE_MEM
		p.From.Reg = v.Args[0].Reg(pstate.ssa)
		pstate.gc.AddAux(&p.From, v)
		p.To.Type = obj.TYPE_REG
		p.To.Reg = arm64.REGTMP
		if pstate.gc.Debug_checknil != 0 && v.Pos.Line() > 1 { // v.Line==1 in generated wrappers
	...
	case ssa.OpARM64Equal,
		...
		p := s.Prog(pstate.gc, arm64.ACSET)
		p.From.Type = obj.TYPE_REG // assembler encodes conditional bits in Reg
		p.From.Reg = pstate.condBits[v.Op]
		p.To.Type = obj.TYPE_REG
		p.To.Reg = v.Reg(pstate.ssa)
...
func (pstate *PackageState) ssaGenBlock(s *gc.SSAGenState, b, next *ssa.Block) {
	...
	case ssa.BlockDefer:
		...
		p := s.Prog(pstate.gc, arm64.ACMP)
		p.From.Type = obj.TYPE_CONST
		p.From.Offset = 0
		p.Reg = arm64.REG_R0
		p = s.Prog(pstate.gc, arm64.ABNE)
		p.To.Type = obj.TYPE_BRANCH
	...
	case ssa.BlockARM64TBZ, ssa.BlockARM64TBNZ:
		...
		p.From.Type = obj.TYPE_CONST
		p.Reg = b.Control.Reg(pstate.ssa)
