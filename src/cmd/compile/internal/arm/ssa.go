...
func (psess *PackageSession) genregshift(s *gc.SSAGenState, as obj.As, r0, r1, r2, r int16, typ int64) *obj.Prog {
	p := s.Prog(psess.gc, as)
	p.From.Type = obj.TYPE_SHIFT
...
func (psess *PackageSession) ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpARMMULAF, ssa.OpARMMULAD, ssa.OpARMMULSF, ssa.OpARMMULSD:
		...
		if r != r0 {
			v.Fatalf("result and addend are not in the same register: %v", v.LongString(psess.ssa))
		}
		p := s.Prog(psess.gc, v.Op.Asm(psess.ssa))
		p.From.Type = obj.TYPE_REG
		p.From.Reg = r2
	...
	case ssa.OpARMSRRconst:
		psess.
			genshift(s, arm.AMOVW, 0, v.Args[0].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_RR, v.AuxInt)
	case ssa.OpARMADDshiftLL,
		...
		ssa.OpARMBICshiftLL:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LL, v.AuxInt)
	case ssa.OpARMADDSshiftLL,
		ssa.OpARMSUBSshiftLL,
		ssa.OpARMRSBSshiftLL:
		p := psess.genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg0(psess.ssa), arm.SHIFT_LL, v.AuxInt)
		p.Scond = arm.C_SBIT
	case ssa.OpARMADDshiftRL,
		...
		ssa.OpARMBICshiftRL:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LR, v.AuxInt)
	case ssa.OpARMADDSshiftRL,
		ssa.OpARMSUBSshiftRL,
		ssa.OpARMRSBSshiftRL:
		p := psess.genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg0(psess.ssa), arm.SHIFT_LR, v.AuxInt)
		p.Scond = arm.C_SBIT
	case ssa.OpARMADDshiftRA,
		...
		ssa.OpARMBICshiftRA:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_AR, v.AuxInt)
	case ssa.OpARMADDSshiftRA,
		ssa.OpARMSUBSshiftRA,
		ssa.OpARMRSBSshiftRA:
		p := psess.genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg0(psess.ssa), arm.SHIFT_AR, v.AuxInt)
		p.Scond = arm.C_SBIT
	case ssa.OpARMXORshiftRR:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_RR, v.AuxInt)
	case ssa.OpARMMVNshiftLL:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LL, v.AuxInt)
	case ssa.OpARMMVNshiftRL:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LR, v.AuxInt)
	case ssa.OpARMMVNshiftRA:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_AR, v.AuxInt)
	case ssa.OpARMMVNshiftLLreg:
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LL)
	case ssa.OpARMMVNshiftRLreg:
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LR)
	case ssa.OpARMMVNshiftRAreg:
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_AR)
	case ssa.OpARMADDshiftLLreg,
		...
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LL)
	case ssa.OpARMADDSshiftLLreg,
		ssa.OpARMSUBSshiftLLreg,
		ssa.OpARMRSBSshiftLLreg:
		p := psess.genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), v.Reg0(psess.ssa), arm.SHIFT_LL)
		p.Scond = arm.C_SBIT
	case ssa.OpARMADDshiftRLreg,
		...
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LR)
	case ssa.OpARMADDSshiftRLreg,
		ssa.OpARMSUBSshiftRLreg,
		ssa.OpARMRSBSshiftRLreg:
		p := psess.genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), v.Reg0(psess.ssa), arm.SHIFT_LR)
		p.Scond = arm.C_SBIT
	case ssa.OpARMADDshiftRAreg,
		...
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_AR)
	case ssa.OpARMADDSshiftRAreg,
		ssa.OpARMSUBSshiftRAreg,
		ssa.OpARMRSBSshiftRAreg:
		p := psess.genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), v.Reg0(psess.ssa), arm.SHIFT_AR)
		p.Scond = arm.C_SBIT
	case ssa.OpARMHMUL,
		ssa.OpARMHMULU:
	...
	case ssa.OpARMMULLU:
		...
		p.To.Type = obj.TYPE_REGREG
		p.To.Reg = v.Reg0(psess.ssa)
		p.To.Offset = int64(v.Reg1(psess.ssa))
	case ssa.OpARMMULA, ssa.OpARMMULS:
		...
		p.To.Reg = v.Reg(psess.ssa)
		p.To.Offset = int64(v.Args[2].Reg(psess.ssa))
	...
	case ssa.OpARMCMPshiftLL, ssa.OpARMCMNshiftLL, ssa.OpARMTSTshiftLL, ssa.OpARMTEQshiftLL:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), 0, arm.SHIFT_LL, v.AuxInt)
	case ssa.OpARMCMPshiftRL, ssa.OpARMCMNshiftRL, ssa.OpARMTSTshiftRL, ssa.OpARMTEQshiftRL:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), 0, arm.SHIFT_LR, v.AuxInt)
	case ssa.OpARMCMPshiftRA, ssa.OpARMCMNshiftRA, ssa.OpARMTSTshiftRA, ssa.OpARMTEQshiftRA:
		psess.
			genshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), 0, arm.SHIFT_AR, v.AuxInt)
	case ssa.OpARMCMPshiftLLreg, ssa.OpARMCMNshiftLLreg, ssa.OpARMTSTshiftLLreg, ssa.OpARMTEQshiftLLreg:
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), 0, arm.SHIFT_LL)
	case ssa.OpARMCMPshiftRLreg, ssa.OpARMCMNshiftRLreg, ssa.OpARMTSTshiftRLreg, ssa.OpARMTEQshiftRLreg:
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), 0, arm.SHIFT_LR)
	case ssa.OpARMCMPshiftRAreg, ssa.OpARMCMNshiftRAreg, ssa.OpARMTSTshiftRAreg, ssa.OpARMTEQshiftRAreg:
		psess.
			genregshift(s, v.Op.Asm(psess.ssa), v.Args[0].Reg(psess.ssa), v.Args[1].Reg(psess.ssa), v.Args[2].Reg(psess.ssa), 0, arm.SHIFT_AR)
	case ssa.OpARMMOVWaddr:
		p := s.Prog(psess.gc, arm.AMOVW)
	...
	case ssa.OpARMMOVWloadshiftLL:
		p := psess.genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LL, v.AuxInt)
		p.From.Reg = v.Args[0].Reg(psess.ssa)
	case ssa.OpARMMOVWloadshiftRL:
		p := psess.genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_LR, v.AuxInt)
		p.From.Reg = v.Args[0].Reg(psess.ssa)
	case ssa.OpARMMOVWloadshiftRA:
		p := psess.genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[1].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_AR, v.AuxInt)
		p.From.Reg = v.Args[0].Reg(psess.ssa)
	...
	case ssa.OpARMMOVWstoreshiftLL:
		...
		p.To.Reg = v.Args[0].Reg(psess.ssa)
		p.To.Offset = int64(makeshift(v.Args[1].Reg(psess.ssa), arm.SHIFT_LL, v.AuxInt))
	case ssa.OpARMMOVWstoreshiftRL:
		...
		p.To.Type = obj.TYPE_SHIFT
		p.To.Reg = v.Args[0].Reg(psess.ssa)
		p.To.Offset = int64(makeshift(v.Args[1].Reg(psess.ssa), arm.SHIFT_LR, v.AuxInt))
	case ssa.OpARMMOVWstoreshiftRA:
		...
		p.To.Type = obj.TYPE_SHIFT
		p.To.Reg = v.Args[0].Reg(psess.ssa)
		p.To.Offset = int64(makeshift(v.Args[1].Reg(psess.ssa), arm.SHIFT_AR, v.AuxInt))
	case ssa.OpARMMOVBreg,
		...
		if a.Op == ssa.OpLoadReg {
			t := a.Type
			switch {
			case v.Op == ssa.OpARMMOVBreg && t.Size(psess.types) == 1 && t.IsSigned(),
				v.Op == ssa.OpARMMOVBUreg && t.Size(psess.types) == 1 && !t.IsSigned(),
				v.Op == ssa.OpARMMOVHreg && t.Size(psess.types) == 2 && t.IsSigned(),
				v.Op == ssa.OpARMMOVHUreg && t.Size(psess.types) == 2 && !t.IsSigned():
		...
		if psess.objabi.GOARM >= 6 {
			psess.
				genshift(s, v.Op.Asm(psess.ssa), 0, v.Args[0].Reg(psess.ssa), v.Reg(psess.ssa), arm.SHIFT_RR, 0)
			return
		}
		fallthrough
	case ssa.OpARMMVN,
	...
	case ssa.OpARMLoweredZero:
		...
		p.To.Reg = arm.REG_R1
		p.To.Offset = sz
		p2 := s.Prog(psess.gc, arm.ACMP)
		p2.From.Type = obj.TYPE_REG
		p2.From.Reg = v.Args[1].Reg(psess.ssa)
		p2.Reg = arm.REG_R1
		p3 := s.Prog(psess.gc, arm.ABLE)
	...
	case ssa.OpARMLoweredMove:
		...
		p2.To.Offset = sz
		p3 := s.Prog(psess.gc, arm.ACMP)
		p3.From.Type = obj.TYPE_REG
		p3.From.Reg = v.Args[2].Reg(psess.ssa)
		p3.Reg = arm.REG_R1
		p4 := s.Prog(psess.gc, arm.ABLE)
...
func (psess *PackageSession) ssaGenBlock(s *gc.SSAGenState, b, next *ssa.Block) {
	...
	case ssa.BlockDefer:
		...
		p := s.Prog(psess.gc, arm.ACMP)
		p.From.Type = obj.TYPE_CONST
		p.From.Offset = 0
		p.Reg = arm.REG_R0
		p = s.Prog(psess.gc, arm.ABNE)
		p.To.Type = obj.TYPE_BRANCH
