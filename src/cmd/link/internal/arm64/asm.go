...
func (psess *PackageSession) adddynrel(ctxt *ld.Link, s *sym.Symbol, r *sym.Reloc) bool {
	...
	log.Fatalf("adddynrel not implemented for relocation type %d (%s)", r.Type, psess.sym.RelocName(ctxt.Arch, r.Type))
	return false
}
...
func (psess *PackageSession) machoreloc1(arch *sys.Arch, out *ld.OutBuf, s *sym.Symbol, r *sym.Reloc, sectoff int64) bool {
	...
	case objabi.R_CALLARM64:
		if r.Xadd != 0 {
			psess.ld.
				Errorf(s, "ld64 doesn't allow BR26 reloc with non-zero addend: %s+%d", rs.Name, r.Xadd)
		}
...
func (psess *PackageSession) archreloc(ctxt *ld.Link, r *sym.Reloc, s *sym.Symbol, val *int64) bool {
		...
		case objabi.R_ARM64_GOTPCREL:
			...
			if !(r.Sym.Version != 0 || r.Sym.Attr.VisibilityHidden() || r.Sym.Attr.Local()) && r.Sym.Type == sym.STEXT && ctxt.DynlinkingGo() {
				if o2&0xffc00000 != 0xf9400000 {
					psess.ld.
						Errorf(s, "R_ARM64_GOTPCREL against unexpected instruction %x", o2)
				}
	...
	case objabi.R_GOTOFF:
		*val = psess.ld.Symaddr(r.Sym) + r.Add - psess.ld.Symaddr(ctxt.Syms.Lookup(".got", 0))
		return true
	case objabi.R_ADDRARM64:
		t := psess.ld.Symaddr(r.Sym) + r.Add - ((s.Value + int64(r.Off)) &^ 0xfff)
		if t >= 1<<32 || t < -1<<32 {
			psess.ld.
				Errorf(s, "program too large, address relocation distance = %d", t)
		}
	...
	case objabi.R_ARM64_TLS_LE:
		...
		if ctxt.HeadType != objabi.Hlinux {
			psess.ld.
				Errorf(s, "TLS reloc on unsupported OS %v", ctxt.HeadType)
		}
	...
	case objabi.R_CALLARM64:
		...
		if t >= 1<<27 || t < -1<<27 {
			psess.ld.
				Errorf(s, "program too large, call relocation distance = %d", t)
		}
		*val |= (t >> 2) & 0x03ffffff
		return true
	}
