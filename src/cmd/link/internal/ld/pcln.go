...
func (psess *PackageSession) numberfile(ctxt *Link, file *sym.Symbol) {
	if file.Type != sym.SFILEPATH {
		ctxt.Filesyms = append(ctxt.Filesyms, file)
		file.Value = int64(len(ctxt.Filesyms))
		file.Type = sym.SFILEPATH
		path := file.Name[len(src.FileSymPrefix):]
		file.Name = psess.expandGoroot(path)
	}
}

func (psess *PackageSession) renumberfiles(ctxt *Link, files []*sym.Symbol, d *sym.Pcdata) {
...
func (ctxt *Link) pclntab(psess *PackageSession) {
	...
	for _, s := range ctxt.Textp {
		...
		pcln := s.FuncInfo
		if pcln == nil {
			pcln = &psess.pclntabZpcln
		}

		if psess.pclntabFirstFunc == nil {
			psess.
				pclntabFirstFunc = s
		}
	...
	if ctxt.Debugvlog != 0 {
		ctxt.Logf("%5.2f pclntab=%d bytes, funcdata total %d bytes\n", psess.Cputime(), ftab.Size, funcdataBytes)
	}
}

func (psess *PackageSession) gorootFinal() string {
	root := psess.objabi.GOROOT
...
func (psess *PackageSession) expandGoroot(s string) string {
	const n = len("$GOROOT")
	if len(s) >= n+1 && s[:n] == "$GOROOT" && (s[n] == '/' || s[n] == '\\') {
		return filepath.ToSlash(filepath.Join(psess.gorootFinal(), s[n:]))
	}
	return s
}
...
func (ctxt *Link) findfunctab(psess *PackageSession) {
