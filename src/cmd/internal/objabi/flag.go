...
func (versionFlag) Set(psess *PackageSession, s string) error {
	name := os.Args[0]
	name = name[strings.LastIndex(name, "/")+1:]
	name = name[strings.LastIndex(name, "\\")+1:]
	name = strings.TrimSuffix(name, ".exe")
	p := psess.Expstring()
	if p == psess.DefaultExpstring() {
		p = ""
	}
	sep := ""
	if p != "" {
	...
	if s == "full" && strings.HasPrefix(psess.Version, "devel") {
		p += " buildID=" + psess.buildID
	}
	fmt.Printf("%s version %s%s%s\n", name, psess.Version, sep, p)
	os.Exit(0)
	return nil
}
