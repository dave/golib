...
func (i RelocType) String(pstate *PackageState) string {
	i -= 1
	if i < 0 || i >= RelocType(len(pstate._RelocType_index)-1) {
		return "RelocType(" + strconv.FormatInt(int64(i+1), 10) + ")"
	}
	return _RelocType_name[pstate._RelocType_index[i]:pstate._RelocType_index[i+1]]
}
